...
#   TEST_QEMU_MEMORY: Specify memory size (KB) for qemu machine. Default: 512 KB
...
for distro in $TEST_DISTROS; do
	...
	for machine in $TEST_MACHINES; do
		...
		if [ -n "$TEST_QEMU_MEMORY" ] && [[ "$TEST_QEMU_MEMORY" =~ ^[0-9]+$ ]] && [ "512" -lt "$TEST_QEMU_MEMORY" ]; then
			note "Set $machine memory to $TEST_QEMU_MEMORY KB."
